...
ggplot(res_gr, aes(x=nk,y=pi1,col=rep_pop)) + geom_point() + geom_line() + facet_wrap(~disc_pop) +
...
ggplot(res_gr, aes(x=nk,y=pi1,col=rep_pop)) + geom_point() + geom_line() + facet_wrap(~disc_pop,nrow=1) +
  scale_color_viridis(discrete = TRUE) + theme_bw(14) + labs(x="Number Peer Factors",y=expression(pi[1]),col="Replication\nPopulation") 
```
...
for(pop in c('afa','cau','his')){
  a <- read.table(my.dir %&% pop %&% '_summary_pcs_nk.txt',header=T) %>% dplyr::mutate(nk=nk)
  if(exists('eqtl')){
    eqtl <- rbind(eqtl, a)
  }else{
    eqtl <- a
...
ggplot(eqtl, aes(x=Nk,y=FDR_0.05,col=pop)) + geom_point() + geom_line() + facet_wrap(~pc,nrow=1) +
  scale_y_continuous(labels = function(x) format(x, scientific = TRUE)) +
  scale_color_viridis(discrete = TRUE) + theme_bw(14) + labs(x="Number Peer Factors",y="Number of eQTLs (FDR < 0.05)")
