...
fig <- ggplot(resg,aes(x=exp,y=obs,col=pop)) + geom_point(size=0.6) + scale_color_viridis(discrete = TRUE) +
  theme_bw(12) + geom_abline(intercept = 0, slope = 1) + labs(x=expression(paste('Expected ',-log[10],' ',P)),
...
```{r}
tiff("Fig6.tiff", width = 9.5, height = 8.1, units = 'cm', res = 300, compression = 'lzw')
fig
dev.off()
```
