...
fig3a <- ggplot(ba, aes(x=test_R2_avg_1,y=diffR2,col=alpha)) + geom_point(shape=1) + 
  ...
  facet_wrap(~pop) + theme_bw(12) + theme(legend.justification=c(0,0), legend.position=c(0.9,0.01),legend.title = element_text(size=10),legend.text = element_text(size=10),legend.key.size = unit(0.4, "cm"))
...
scale_color_viridis(discrete=TRUE) + labs(x='BSLMM PVE',y='PVE difference (BSLMM - model)',title='B') +
  ...
  theme_bw(12) + theme(legend.justification=c(0,0), legend.position=c(0.88,0.05),legend.title = element_text(size=10),legend.text = element_text(size=10),legend.key.size = unit(0.3, "cm"),legend.background = element_rect(fill="white",size=0.5, linetype="solid", colour ="black"))
...
```{r}
tiff("Fig3.tiff", width = 19, height = 16.6, units = 'cm', res = 300, compression = 'lzw')
grid.arrange(fig3a, fig3b, nrow=2)
dev.off()
```