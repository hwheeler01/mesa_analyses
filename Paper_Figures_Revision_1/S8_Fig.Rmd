...
fig <- ggplot(newall, aes(x=beta_Fst,fill=diffGroup)) + geom_density(alpha=0.7)+ facet_wrap(~thres,nrow=1) +
 labs(x=expression(paste("weighted ", F[ST])),fill=expression(paste("|",R^2, " difference|"))) + theme_bw(12) +  theme(plot.margin=unit(c(0,0.2,0,0.2), "cm")) + scale_fill_viridis(discrete = TRUE) + coord_cartesian(xlim=c(-0.001,0.01))+
  theme(legend.justification=c(0,1), legend.position=c(0.85,0.99),legend.title = element_text(size=8),legend.text = element_text(size=8),legend.key.size = unit(0.6, "cm"),plot.margin=unit(c(0,0.2,0,0.2), "cm"),axis.text.x = element_text(angle = 90, hjust = 1))
print(fig)
```


```{r}
tiff("S8_Fig.tiff", width = 7.5, height = 2.8, units = 'in', res = 300, compression = 'lzw')
fig
dev.off()
```
