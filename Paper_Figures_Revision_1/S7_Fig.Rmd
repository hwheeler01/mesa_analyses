...
dt_down_hc <- data.table(down_hc)
dt_down_hc[,list(R=cor(allsample,downsample,use='p',method='s')),by=p]
s7a <- ggplot(down_hc, aes(x=downsample,y=allsample)) + geom_point(shape=1) + facet_wrap(~p) + geom_abline(slope=1,intercept=0,col='blue') +
  theme_bw(12) + labs(title='A',x=expression(paste('Downsampled ', R^2, ' (n = 233)')),
...
dt_suball_nof_dw <- data.table(suball_nof_dw)
dt_suball_nof_dw[,list(R=cor(pop1,pop2,use='p',method='s')),by=pop]
s7b <- ggplot(suball_nof_dw, aes(x=pop1,y=pop2)) + geom_point(shape=1) + geom_abline(slope=1,intercept=0,col='blue') + facet_wrap(~pop,nrow=1) + theme_bw(12)+
...
plot_grid(s7a, s7b, align = "v", axis="tb", nrow = 2, rel_heights = c(4/8,3/8))
```

```{r}
tiff("S7_Fig.tiff", width = 7, height = 7.5, units = 'in', res = 300, compression = 'lzw')
plot_grid(s7a, s7b, align = "v", axis="tb", nrow = 2, rel_heights = c(4/8,3/8))
dev.off()
```
