...
a <- ggplot(all,aes(x=PF10,y=PF0)) + geom_point(shape=1) + facet_wrap(~pop) + theme_bw(12) + labs(x=expression(paste(R^2,' with 10 PEER Factors')),y=expression(paste(R^2,' with 0 PEER Factors'))) +
  geom_abline(slope=1,intercept=0,col='blue')

b <- ggplot(all,aes(x=PF10,y=PF20)) + geom_point(shape=1) + facet_wrap(~pop) + theme_bw(12) +labs(x=expression(paste(R^2,' with 10 PEER Factors')),y=expression(paste(R^2,' with 20 PEER Factors'))) +
  geom_abline(slope=1,intercept=0,col='blue')

c <- ggplot(all,aes(x=PF10,y=PF30)) + geom_point(shape=1) + facet_wrap(~pop) + theme_bw(12) +labs(x=expression(paste(R^2,' with 10 PEER Factors')),y=expression(paste(R^2,' with 30 PEER Factors'))) +
  geom_abline(slope=1,intercept=0,col='blue')
...
```{r}
tiff("S6_Fig.tiff", width = 7.5, height = 8.44, units = 'in', res = 300, compression = 'lzw')
grid.arrange(a,b,c)
dev.off()
```
